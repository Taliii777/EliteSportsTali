...
const ChatBot: React.FC<ChatBotProps> = ({ isOpen, setIsOpen }) => {
  ...
  const renderTextWithLinks = (text: string) => {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (type === 'email') {
          ...
          const beforeMatch = text
            ...
            .toLowerCase();
          const isWhatsApp =
            beforeMatch.includes('whatsapp') || beforeMatch.includes('💬');
    ...
    nonOverlappingMatches.forEach((match, i) => {
      ...
      const isExternalLink =
        match.type === 'instagram' ||
        (match.type === 'phone' && match.url.startsWith('https://wa.me'));
      parts.push(
        <a
          key={`link-${i}`}
          href={match.url}
  ...
  const handleBotResponse = (userInput: string): Message => {
    ...
    // Athlete sub-options
    if (input === 'apply for representation') {
      response = {
        id: messageId,
    ...
    } else if (input === 'sponsorship & partnerships') {
      response = {
        id: messageId,
    ...
    } else if (input === 'social media & branding help') {
      response = {
        id: messageId,
    ...
    // Brand sub-options
    else if (input === 'sponsor an athlete') {
      response = {
        id: messageId,
    ...
    } else if (input === 'sponsor an event') {
      response = {
        id: messageId,
    ...
    } else if (input === 'enter the padel market') {
      response = {
        id: messageId,
    ...
    } else if (input === 'create a custom activation') {
      response = {
        id: messageId,
    ...
    } else if (input === 'see partnership examples') {
      response = {
        id: messageId,
    ...
    // Club sub-options
    else if (input === 'secure sponsorship') {
      response = {
        id: messageId,
    ...
    } else if (input === 'build partnerships') {
      response = {
        id: messageId,
    ...
    } else if (input === 'bring athletes to the club') {
      response = {
        id: messageId,
    ...
    // Event sub-options
    else if (input === 'what type of event?') {
      response = {
        id: messageId,
    ...
    } else if (input === 'looking for athletes?') {
      response = {
        id: messageId,
    ...
    } else if (input === 'looking for sponsors?') {
      response = {
        id: messageId,
    ...
    } else if (input === 'need a deck or concept?') {
      response = {
        id: messageId,
    ...
    } else if (input === 'what makes esm unique?') {
      response = {
        id: messageId,
    ...
    // Contact form option
    else if (input === 'contact form' || input.includes('contact form')) {
      response = {
        id: messageId,
    ...
    // Contact options
    else if (input === 'send email' || input === 'email') {
    ...
    } else if (input === 'open whatsapp' || input === 'whatsapp') {
      const message = encodeURIComponent(
        "Hi, I'd like to learn more about ESM's services.",
      );
    ...
    } else if (input === 'open instagram' || input === 'instagram') {
      window.open(
        'https://www.instagram.com/elitesportsmanagement__/',
        '_blank',
      );
      response = {
        id: messageId,
    ...
    } else if (input === 'contact esm' || input === 'contact') {
      response = {
        id: messageId,
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => {
        ...
        if (
          lastBotMessage &&
          lastBotMessage.isBot &&
          lastBotMessage.text === botResponse.text
        ) {
          return prev;
        }
  ...
  return (
    <div
      className={`fixed bottom-0 right-0 z-50 w-full sm:w-80 h-[450px] bg-light
        shadow-[0_-8px_32px_rgba(0,0,0,0.15),-5px_0_15px_rgba(0,0,0,0.1),5px_0_15px_rgba(0,0,0,0.1)]
    ...
    >
      ...
      <div className='flex items-center justify-between p-3 bg-darkBlue text-light sm:rounded-t-2xl'>
        <div className='flex items-center gap-2'>
          ...
          <h3 className='font-semibold font-neue-roman text-sm'>
            ESM Assistant
          </h3>
        </div>
        <button
